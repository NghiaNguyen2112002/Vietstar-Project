...
# 34 "eeprom.c"
void EEP_SaveAutoTime(void){
    eeprom_write(0x00, _timeAuto);
    eeprom_write(0x01, _timeAuto >> 8);
}

void EEP_SaveCupTotal(void){
    eeprom_write(0x02, _cupTotal);
    eeprom_write(0x03, _cupTotal >> 8);
}

void EEP_ReadAutoTime(void){
    _timeAuto = eeprom_read(0x00) | eeprom_read(0x01) << 8;
    if(_timeAuto > 9900) _timeAuto = 9900;
}

void EEP_ReadCupTotal(void){
    _cupTotal = eeprom_read(0x02) | eeprom_read(0x03) << 8;
}
